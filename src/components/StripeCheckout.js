...
const CheckoutForm = () => {
  ...
  const [processing, setProcessing] = useState(false); // Changed initial state to false
  ...
  const createPaymentIntent = async () => {
    try {
      const { data } = await axios.post(
        ...
        JSON.stringify({ cart, shipping_fee, total_amount }) // Ensure you're passing the data correctly here
      );
      console.log(data.clientSecret);
      setClientSecret(data.clientSecret);
    } catch (error) {
      error.response && setError(error.response.data.msg);
    }
  };
